...
import {
  Box, Heading, HStack, Button, Table, Thead, Tr, Th, Tbody, Td,
  Select, NumberInput, NumberInputField, useToast, Spinner, Text,
...
export default function PurchasesPage() {
  ...
  return (
    <Box p={6}>
      <Heading size="lg" mb={4}>Compras</Heading>

      {(loadingClients || loadingItems || loadingPurch) && (
        <HStack mb={4}><Spinner /><Text>Cargando…</Text></HStack>
      )}
      ...
      <Box bg="white" rounded="md" shadow="sm" p={4} mb={6}>
        <Heading size="sm" mb={3}>Registrar compra</Heading>
      ...
      <Box bg="white" rounded="md" shadow="sm" overflowX="auto">
        {isFetching && (
          <Text fontSize="sm" color="gray.500" p={3}>Actualizando…</Text>
        )}
        <Table>
          <Thead>
            <Tr>

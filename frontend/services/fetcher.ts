  ...
  if (isBrowser) {  
    try {  
      ...
      if (rawAuth) {  
        ...
        const userId: string | undefined = parsed?.user?.id; // Específicamente user.id  
          
        if (token) {  
          (config.headers as AxiosHeaders).set("Authorization", `Bearer ${token}`);  
          // eslint-disable-next-line no-console  
          console.log("[fetcher] Enviando Authorization header");  
        }  
          
        // Fuerza el envío del X-User-Id con debug mejorado  
        if (userId) {  
          (config.headers as AxiosHeaders).set("X-User-Id", userId);  
          // eslint-disable-next-line no-console  
          console.log("[fetcher] Enviando X-User-Id:", userId);  
        } else {  
          // eslint-disable-next-line no-console  
          console.warn("[fetcher] userId no encontrado en localStorage. Estructura:", parsed);  
        }  
      } else {  
        // eslint-disable-next-line no-console  
        console.warn("[fetcher] No se encontró 'auth' en localStorage");  
      }  
    } catch (error) {  
      // eslint-disable-next-line no-console  
      console.error("[fetcher] Error parseando localStorage auth:", error);  
    }  
  }  
  return config;  
});  
